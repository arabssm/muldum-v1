...
export default function Month() {
  ...
  const [schedules, setSchedules] = useState<
    { startDate: string; endDate: string; text: string; color: string }[]
  >([]);

  const save = (startDate: string, endDate: string, text: string) => {
    setSchedules((prev) => [
      ...prev,
      { startDate, endDate, text, color: COLORS[colorIndex++ % COLORS.length] },
    ]);
    setOpen(false);
    setSel(null);
  };
  ...
  return (
    <_.Container>
      ...
      <_.CalendarContainer>
        <_.CalendarHeader>
          ...
          <_.NavButtonGroup>
            ...
            <_.NavButton onClick={() => setDate(dayjs())} />
        ...
        <_.Weekdays>
          {["일", "월", "화", "수", "목", "금", "토"].map((d) => (
            <_.WeekdayItem key={d}>{d}</_.WeekdayItem>
          ))}
        </_.Weekdays>
        ...
        <_.Dates>
          {Array.from({ length: total }, (_, i) => {
            ...
            const events = schedules.filter(
              (s) =>
                dayjs(full).isSameOrAfter(s.startDate, "day") &&
                dayjs(full).isSameOrBefore(s.endDate, "day")
            );
            return (
              <DateCell
                key={i}
                inactive={!inMonth}
                onClick={() => inMonth && (setSel(full), setOpen(true))}
              >
                {inMonth && d}
                {events.map((s, idx) => (
                  <EventBadge
                    key={idx}
                    color={s.color}
                    title={`${s.text} (${s.startDate} ~ ${s.endDate})`}
                  >
                    {s.text}
                  </EventBadge>
                ))}
      ...
      {open && sel && (
        <ScheduleModal onClose={() => setOpen(false)} onSave={save} initialStartDate={sel} />
      )}
    </_.Container>
  );
}
